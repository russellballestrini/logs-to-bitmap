...
class TestAnomalyDetection(unittest.TestCase):
    ...
    def test_contamination_parameter_effect(self):
        ...
        detector_1pct = RequestBitmapAnomalyDetector(contamination=0.01, random_state=42)
        detector_5pct = RequestBitmapAnomalyDetector(contamination=0.05, random_state=42)
    ...
    def test_temporal_pattern_isolation(self):
        ...
        for i in range(50):
            timestamp = (base_time + datetime.timedelta(seconds=i)).strftime('%Y%m%d_%H%M%S_%f')
        ...
        try:
            for i in range(50):
                timestamp = (late_base + datetime.timedelta(seconds=i)).strftime('%Y%m%d_%H%M%S_%f')
            ...
            self.assertLess(anomalies, 5, "Too many anomalies detected due to temporal drift")
