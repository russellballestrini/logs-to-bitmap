...
class TestTemporalDrift(unittest.TestCase):
    ...
    def create_dataset_with_timestamps(self, output_dir, base_time, num_requests=50, anomaly_positions=None):
    ...
    def test_temporal_drift_impact(self):
        ...
        correctly_detected = sum(1 for pos in detected_positions if pos in known_anomalies)
        
        # Calculate accuracy metrics
        precision = correctly_detected / len(detected_positions) if detected_positions else 0
        ...
        self.assertGreater(recall, 0.2, "Temporal drift caused complete detection failure")
        
        # Too many false positives indicates temporal overfitting
        self.assertLess(len(detected_positions), 15, "Too many false positives - temporal overfitting suspected")
...
class TestFormatSampling(unittest.TestCase):
    ...
    def create_multi_format_dataset(self, output_dir, num_requests=20, anomaly_positions=None):
        ...
        for i in range(num_requests):
            ...
            for fmt, ext in [('BMP', 'bmp'), ('JPEG', 'jpg'), ('PNG', 'png'), ('WebP', 'webp')]:
    ...
    def test_format_sampling_consistency(self):
        ...
        self.create_multi_format_dataset(
            dataset_dir, 
            num_requests=20, 
            anomaly_positions=known_anomalies
        )
        ...
        for seed, result in results_by_seed.items():
            print(f"Seed {seed}: detected={result['detected']}, recall={result['recall']:.2f}")
        ...
        print(f"Format sampling recall: min={min_recall:.2f}, max={max_recall:.2f}, avg={avg_recall:.2f}")
        ...
        self.assertLess(max_recall - min_recall, 0.6, "Too much variance between format sampling runs")
...
class TestOverfittingDetection(unittest.TestCase):
    ...
    def test_cross_validation_consistency(self):
        ...
        correctly_detected = sum(1 for pos in detected_positions if pos in fold2_anomalies)
        ...
        self.assertGreater(recall, 0.1, "Poor cross-validation performance suggests overfitting")
